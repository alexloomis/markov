...
instance Combine FromLists where
  combine = const

instance Markov ((,) (Product Double)) FromLists where
  transition =
    let mat = [[0.4, 0.3, 0.3], [0.2, 0.1, 0.7], [0.9, 0.1, 0.0]]
...
instance Markov ((,) (Product Double)) Simple where
  transition _ = [0.5 >*< pred, 0.5 >*< succ]

instance Markov ((,) (Product Int)) Simple where
  transition _ = [1 >*< pred, 1 >*< succ]

instance Markov ((,) (Sum Int)) Simple where
  transition _ = [1 >*< pred, 0 >*< id, 0 >*< succ]
...
instance Combine Urn where
  combine = const

instance Markov ((,) (Product Double)) Urn where
  transition x = [probLeft x >*< addLeft, 1 - probLeft x >*< addRight]
...
instance Combine Tidal where
  combine = const

instance Markov ((,) (Product Double)) Tidal where
  transition tw =
    [probRight tw >*< stepPos (+ 1), 1 - probRight tw >*< stepPos (flip (-) 1)]
...
probRight tw = Product $ timeBias * positionBias
  where
    timeBias = (1 + sin (2 * pi * time tw / stepsPerCycle)) / 2
    positionBias
...
instance Markov ((,) (Product Rational, Merge String)) Room where
  sequential = [giveToken, changeState]
    where
      ...
      giveToken =
        \case
          1 -> [0.5 >*< Merge "a" >*< const 1, 0.5 >*< Merge "b" >*< const 1]
          2 -> [0.3 >*< Merge "a" >*< const 2, 0.7 >*< Merge "b" >*< const 2]
          3 ->
            [ 0.4 >*< Merge "a" >*< const 3
            , 0.4 >*< Merge "b" >*< const 3
            , 0.2 >*< Merge "c" >*< const 3
...
instance Markov ((,) (Product Double)) FillBin where
  transition x =
    case probId x of
      0 ->
        filter (\(Product y, _) -> y /= 0) -- Careful, Product _ == Product _ = True
         $
...
probGrowL i x
  ...
  | otherwise = 0
  where
    test = i == 1 || fullN i x < fullN (i - 1) x
...
probGrowR i x
  ...
  | otherwise = 0
  where
    test = i == size x || fullN i x <= fullN (i + 1) x
...
-- 2.0
expectedLoss ::
     (Fractional a, Markov ((,) (Product a)) FillBin)
  => [Product a :* FillBin]
  -> a
expectedLoss xs = sum . map probLoss $ chain xs !! idx
  where
    idx = slots . snd . head $ xs
